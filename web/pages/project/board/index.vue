							...
							<v-row>
								<v-col v-for="(module, moduleIndex) in modules" :key="moduleIndex" cols="12" lg="3" md="4">
									<v-item #default="{ active, toggle }">
										<v-card @click="toggle">
											<v-img :aspect-ratio="16/9" :src="module.img">
												<v-expand-transition>
...
export default {
  data () {
    return {
      ...
      required: v => v != null || 'Champ requis',
  ...
  computed: {
    ...mapState({
      selectedBoard: state => state.project.selectedBoard,
    }),
  },
  ...
  watch: {
    selectedBoard: {
      handler: async function (selectedBoard) {
        try {
          this.$nextTick(async () => {
