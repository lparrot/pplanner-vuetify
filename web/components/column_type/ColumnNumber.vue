    ...
    <template #input>
      <validation-observer ref="validator">
        <validation-provider #default="{invalid, errors}" :rules="{required: field.props.required === true}" name=" " slim>
          <v-text-field v-model.trim="tempItem" :error="invalid" :error-messages="errors[0]" :step="field.props.step || 1" clearable type="number"></v-text-field>
        </validation-provider>
      </validation-observer>
    </template>
  </v-edit-dialog>
...
@Component
export default class PpColumnNumber extends mixins(tableColumnMixin) {
  @Ref('validator')
