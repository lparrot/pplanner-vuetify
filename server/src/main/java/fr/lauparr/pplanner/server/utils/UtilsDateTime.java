...
public abstract class UtilsDateTime {
  ...
  public static LocalDate convertToLocalDate(Date date) {
    LocalDateTime dateTime = UtilsDateTime.convertToLocalDateTime(date);
    if (dateTime == null) {
      return null;
    }
  ...
  private static String formatEllapsedTimeDuration(Duration duration) {
    if (duration != null) {
      ...
      String formattedHeures = heures > 0 ? heures + UtilsDateTime.getPlurialString(heures, " heure") : "";
      String formattedMinutes = minutes > 0 ? minutes + UtilsDateTime.getPlurialString(minutes, " minute") : "";
      String formattedSecondes = secondes > 0 || heures <= 0 ? secondes + UtilsDateTime.getPlurialString(secondes, " seconde") : "";
